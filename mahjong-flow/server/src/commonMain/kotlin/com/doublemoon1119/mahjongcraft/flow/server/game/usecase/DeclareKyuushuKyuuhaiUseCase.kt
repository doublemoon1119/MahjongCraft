 ...
 *
 * 這是「流局判定」系列子項的第二塊：接上
 * [com.doublemoon1119.mahjongcraft.logic.rules.riichi.RiichiLegalActionValidator] 早就已經有的
 ...
 * 連莊、不結算任何點數、供託延續到下一局（不呼叫 `collectStickPot`）——不需要新增任何
 * [com.doublemoon1119.mahjongcraft.logic.module.MahjongRuleModule] 規則鉤子，直接把
 * `getLegalActions` 回傳的 [GameAction.ExhaustiveDraw] 實例本身（含其規則無關的 `reason` 屬性）
        ...
        val outcome = gameRepository.update(gameId) { state ->
            when {
                ...
                state.currentPlayer.hand.lastDrawn == null ->
                    ...
                    // 自動得出「連莊」的結果，不需要額外分支。
                    val updatedPlayers = state.players.map { it.recordAction(GameAction.ExhaustiveDraw(exhaustiveDraw.reason)) }
