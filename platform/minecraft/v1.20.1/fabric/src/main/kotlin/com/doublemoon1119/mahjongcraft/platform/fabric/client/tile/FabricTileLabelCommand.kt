...
class FabricTileLabelCommand(
    ...
    /**
     * 組合切換訊息：前綴 + 切換前狀態 → 切換後狀態，「開啟」亮綠、「關閉」亮紅，比照
     * [com.doublemoon1119.mahjongcraft.platform.fabric.server.notification.FabricPlayerFeedbackPublisher]
     * 的 `readyToggledMessage` 配色慣例。
     */
    private fun tileLabelsToggledMessage(enabled: Boolean): MutableText {
        val fromKey = if (enabled) MinecraftMessageKeys.TILE_LABELS_STATE_OFF else MinecraftMessageKeys.TILE_LABELS_STATE_ON
        val fromColor = if (enabled) Formatting.RED else Formatting.GREEN
        val toKey = if (enabled) MinecraftMessageKeys.TILE_LABELS_STATE_ON else MinecraftMessageKeys.TILE_LABELS_STATE_OFF
