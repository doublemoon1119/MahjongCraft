...
class GameActionDisplayTextTest {
    ...
    /** 驗證帶牌面的動作（例如打出）組出「動作 + 牌面」文字，牌面本身也正確解析。 */
    @Test
    fun `discard action includes resolved tile as argument`() {
        val content = GameAction.Discard(Uuid.random()).toDisplayText(fiveDot, registry).content as TranslatableTextContent
        assertEquals("mahjongcraft.message.game_action_discard", content.key)
        val tileArgContent = (content.args.single() as Text).content as TranslatableTextContent
    ...
    fun `tsumo action does not require a reference tile`() {
        val content = GameAction.Tsumo.toDisplayText(null, registry).content as TranslatableTextContent
    ...
    fun `each kan type maps to its own key`() {
        val openKey = (GameAction.Kan(GameAction.KanType.OPEN_KAN, Uuid.random(), emptyList()).toDisplayText(fiveDot, registry).content as TranslatableTextContent).key
        val closedKey = (GameAction.Kan(GameAction.KanType.CLOSED_KAN, Uuid.random(), emptyList()).toDisplayText(fiveDot, registry).content as TranslatableTextContent).key
        val addedKey = (GameAction.Kan(GameAction.KanType.ADDED_KAN, Uuid.random(), emptyList()).toDisplayText(fiveDot, registry).content as TranslatableTextContent).key
    ...
    fun `kyuushu kyuuhai resolves its dedicated key`() {
        val content =
            GameAction.ExhaustiveDraw(RiichiExhaustiveDrawReason.KyuushuKyuuhai).toDisplayText(null, registry).content as TranslatableTextContent
    ...
    fun `other exhaustive draw reasons fall back to the generic key`() {
        ...
        val content = GameAction.ExhaustiveDraw(otherReason).toDisplayText(null, registry).content as TranslatableTextContent
