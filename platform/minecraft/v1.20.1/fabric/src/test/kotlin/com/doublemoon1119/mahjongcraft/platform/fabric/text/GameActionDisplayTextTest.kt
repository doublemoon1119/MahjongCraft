...
class GameActionDisplayTextTest {
    ...
    fun `discard action includes resolved tile as argument`() {
        val content = GameAction.Discard(Uuid.random())
            .toDisplayText(fiveDot, actionDisplayNameRegistry, displayNameRegistry, assetRegistry, emojiRegistry, exhaustiveDrawReasonDisplayNameRegistry).content as TranslatableTextContent
    ...
    fun `kyuushu kyuuhai resolves its dedicated key`() {
        val content = GameAction.ExhaustiveDraw(RiichiExhaustiveDrawReason.KyuushuKyuuhai)
            .toDisplayText(null, actionDisplayNameRegistry, displayNameRegistry, assetRegistry, emojiRegistry, exhaustiveDrawReasonDisplayNameRegistry).content as TranslatableTextContent
